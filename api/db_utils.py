	...
	if metrics is not None:
		...
		for key, value in metrics.items():
			...
			if not new_key[0].isdigit():
				print("convertendo ", new_key, value, "\n")

				if(new_key in ['Symbol', 'timestamp', 'type']):
					new_data_dict[new_key] = value
				else:
					new_data_dict[new_key] = convert_to_float(value.replace('%', '').replace(',', ''))
...
def create_table(cursor, table_name, columns):
	...
	if not table_exists:
		...
		cursor.execute(f"CREATE TABLE IF NOT EXISTS {table_name} ({table_declaration})")
